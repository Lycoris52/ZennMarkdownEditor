...
**このプラグインは独立した非公式プロジェクトであり、Zenn.dev とは一切関係なく、承認・提携・公式な関与もありません。**
...
1. `markdown-post-editor` フォルダを zip 化しますか、もしくは create-plugin-zip.sh で作成します。
...
このプラグインは、実行時に以下のサードパーティ製アセットを読み込みます。

- Shiki によるシンタックスハイライト用: `https://esm.sh`
- KaTeX の JavaScript / CSS / フォント用: `https://cdn.jsdelivr.net`

これらのアセットのバージョンは `markdown-post-editor/markdown-post-editor.php` で固定しています。
...
**This plugin is an independent project and has no affiliation with, endorsement from, or official relationship to Zenn.dev.**
...
1. Zip the `markdown-post-editor` folder, or use the create-plugin-zip.sh to create one.
2. In WordPress admin, open `Plugins > Add New > Upload Plugin`.
3. Upload the zip file and activate it.

## External Runtime Assets

The plugin loads the following third-party assets at runtime:

- `https://esm.sh` for Shiki code syntax highlighting
- `https://cdn.jsdelivr.net` for KaTeX JavaScript, CSS, and font files

These assets are version-pinned in `markdown-post-editor/markdown-post-editor.php`:
...
- Syntax highlighting and math rendering also depend on runtime asset loading from `esm.sh` and `cdn.jsdelivr.net`.
